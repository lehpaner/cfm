...
namespace cfm::application {
	...
	public:
		...
		DWORD Start(void* arg = NULL) {
			...
			m_ThreadCtx.m_hThread = CreateThread(NULL, 0, m_pThreadFunc, this, 0 /*Parti subito*/, &m_ThreadCtx.m_dwTID);
		...
		//REGISTER CALLBACK
		// void RegisterCallback(void callback(void *data, int iEvt, int idThread))
		 //{
		  // pCallback = callback;
		 //}

		bool SetPriority(int nPriority) {
			return (SetThreadPriority(m_ThreadCtx.m_hThread, nPriority)) ? true : false;
		}
	...
	private:
		...
		public:
			...
			unsigned long	m_dwTID;			/**<	The Thread ID */
			void*           m_pUserData;		/**<	The user data pointer */
			void*           m_pParent;			/**<	The this pointer of the parent CThread object */
			unsigned long	m_dwExitCode;		/**<	The Exit Code of the thread */
		};
